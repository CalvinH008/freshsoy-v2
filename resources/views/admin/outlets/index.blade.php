...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Outlet Management</h2>
    ...
    {{-- Search & Filter --}}
    <form action="{{ route('admin.outlets.index') }}" method="GET" class="flex gap-3 mb-6 bg-white p-4 rounded-lg shadow">
            ...
            <tbody>
                @forelse ($outlets as $outlet)
                    <tr class="border-b hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-gray-500">{{ $outlet->code }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $outlet->address }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('admin.outlets.toggle', $outlet) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
